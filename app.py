...
st.set_page_config(page_title="Rastreador de Gastos", page_icon="💰", layout="centered")
...
def load_data():
    ...
    try:
        ...
        return data if not data.empty else pd.DataFrame(columns=["Fecha", "Categoría", "Monto", "Nota"])
    except Exception:
...
if not df_expenses.empty and "Monto" in df_expenses.columns:
    df_expenses["Monto"] = pd.to_numeric(df_expenses["Monto"], errors='coerce').fillna(0)
